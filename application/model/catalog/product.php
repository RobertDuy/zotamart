...
class ModelCatalogProduct extends Model {
    ...
    public function getRelatedProduct($category_id, $product_id){
        $query = $this->db->query("SELECT DISTINCT *, p.product_id, pd.name AS name, p.image, p.price FROM product p LEFT JOIN product_description pd ON p.product_id = pd.product_id
                        WHERE p.isPromotionProduct = 0 AND p.product_id <> ". (int)$product_id . " AND
                        p.product_id in (SELECT product_id FROM `". DB_PREFIX . "product_to_category` WHERE category_id = ". (int)$category_id .")");
        return $query->rows;
    }
    ...
    public function getProductByProductId($product_id){
        $query = $this->db->query("SELECT p.product_id, pd.name, pd.description, p.image, p.category_id, p.price
